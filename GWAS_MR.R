...
exp_raw <- fread("./plink_mac_20231211/data/koges_bmi.txt.gz")
...
out_raw <- fread("./plink_mac_20231211/data/hum0197.v3.BBJ.T2D.v1/GWASsummary_T2D_Japanese_SakaueKanai2020.auto.txt.gz",
                 select=c("SNPID","Allele1","Allele2","BETA","SE","p.value","N","AF_Allele2"))

out_raw$phenotype <- "T2D"

write.csv(out_raw, 'out_raw.csv')
out_file <- ("out_raw.csv")

# out_dat <- TwoSampleMR::format_data( out_raw,
#                         type = "outcome",
#                         snp_col = "SNPID",
#                         beta_col = "BETA",
#                         se_col = "SE",
...
harm_dsub = harmonized_data[c(1:10,977:987,1955:1964),]

res <- mr(harmonized_data)
het=mr_heterogeneity(harmonized_data)
plei=mr_pleiotropy_test(harmonized_data)
...
res_single_sub=mr_singlesnp(harm_dsub)
p2 <- mr_forest_plot(res_single_sub)
p2

res_loo_sub=mr_leaveoneout(harm_dsub)
p3 <- mr_leaveoneout_plot(res_loo_sub)
p3

p4 <- mr_funnel_plot(res_single)
p4

harmonized_data$"r.outcome" <- get_r_from_lor(harmonized_data$"beta.outcome",
